<script lang="ts">
  ...
  const FOOORMA_COMPLETIONS: Completion[] = [
    ...
    // 3D shapes
    snippetCompletion("cube(${x}, ${y}, ${size})",
      { label: 'cube', detail: 'x, y, size, color?, opacity?', type: 'function', boost: 10 }),
    snippetCompletion("sphere(${x}, ${y}, ${size})",
      { label: 'sphere', detail: 'x, y, size, color?, opacity?', type: 'function', boost: 10 }),
    snippetCompletion("cylinder(${x}, ${y}, ${w}, ${h})",
      { label: 'cylinder', detail: 'x, y, w, h, color?, opacity?', type: 'function', boost: 10 }),
    snippetCompletion("torus(${x}, ${y}, ${size})",
      { label: 'torus', detail: 'x, y, size, color?, opacity?', type: 'function', boost: 10 }),
    // Groups
    ...
    snippetCompletion("transform({ rotate: ${0} })",
      { label: 'transform', detail: '{ rotate?, scale?, rotateX?, depth?, ... }', type: 'function', boost: 9 }),
    snippetCompletion("material('${metal}')",
      { label: 'material', detail: "kind, roughness?, intensity?", type: 'function', boost: 8 }),
    // Effects
